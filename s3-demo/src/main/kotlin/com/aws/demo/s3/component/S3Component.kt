...
class S3Component(
    ...
    fun init() {
        ...
        val prefixDeleteRule = LifecycleRule.builder()
            ...
            .expiration(
                LifecycleExpiration.builder().days(30).build() // 30일 후 만료
            )
            .status(ExpirationStatus.ENABLED)
            .build()

        val lifecycleRulesuleList: MutableList<LifecycleRule> = mutableListOf();
        // Tag 를 가진 객체 삭제하도록 설정
        for (i in listOf(1, 10, 15, 30)) {
            lifecycleRulesuleList.add(
                LifecycleRule.builder()
                    ...
                    .expiration(
                        LifecycleExpiration.builder().days(i).build() // i일 후 만료
                    )
                    .status(ExpirationStatus.ENABLED)
                    .build()
            )
        ...
        lifecycleRulesuleList.add(prefixDeleteRule)
        lifecycleRulesuleList.add(transitionRule)
        // Lifecycle 규칙 설정
        val configuration = BucketLifecycleConfiguration.builder()
            .rules(lifecycleRulesuleList)
            .build()
